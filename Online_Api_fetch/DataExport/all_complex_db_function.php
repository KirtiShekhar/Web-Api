...
WHERE 1 = 1 AND bak_pis_personnel.OfficerId = bak_pis_qualification.OfficerId AND bak_pis_personnel.OfficerId = bak_pis_posting.OfficerId AND bak_pis_personnel.OfficerId = bak_pis_central_deputation_info.OfficerID  AND bak_pis_personnel.OfficerId = bak_pis_debarmentinfo.OfficerID  AND bak_pis_personnel.OfficerId = bak_pis_languageinfo.OfficerID ";

if(isset($_GET['Gender_name']))
{
	$Gender_Name = $_GET['Gender_name'];
	$MainSql.= " AND Gender_name = '".$Gender_Name ."'";
}


$MainSql.= " GROUP BY bak_pis_personnel.OfficerId ASC ORDER BY FirstName ASC";
echo $MainSql,'<br/><br/>';
$AllComplexData_Result = mysqli_query($AllComplexData_connection,$MainSql) or die("error".mysqli_error($AllComplexData_connection));

	return $AllComplexData_Result;
}
