...
$officerminorfieldexperienceresult = officerminorfieldexperiencefetch_display_all_field_minor_experience_names();

while ($officerfieldexperiencerows = mysqli_fetch_array($officerminorfieldexperienceresult)) 
{
	$OfficerFieldExperienceType = array();
	$OfficerFieldExperienceType['FieldExpDesc'] = $officerfieldexperiencerows['FieldExpDesc'];
	array_push($officerfieldexperienceresponse, $OfficerFieldExperienceType);
}
print(json_encode(array("status" => "true" , "message" => "Successfully displayed the Major Field Experience Type Names!" , "MinorFieldExperience" => $officerfieldexperienceresponse),JSON_PRETTY_PRINT));
